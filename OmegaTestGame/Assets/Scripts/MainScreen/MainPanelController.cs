...
namespace Scripts.MainScreen
{
    public class MainPanelController : MonoBehaviour
    {
        ...
        [Header("Disabled buttons")] [SerializeField]
        private Button[] _buttons;

        [Header("Music")] [SerializeField] private AudioClip _menuMusic;
            ...
            switch (_state)
            {
                case State.SettingsOpened:
                    _rectTransform.anchoredPosition = Vector2.Lerp(_rectTransform.anchoredPosition,
                        new Vector2(-Screen.width, 0), 5 * Time.deltaTime);
                    break;
        ...
        private void SetState(State state)
        {
            ...
            switch (state)
            {
                ...
                case State.Play:
                    _fader.FadeOut();
                    ActiveButtons(false);
                    SceneManager.Instance.LoadSceneFromBundle("https://s3.amazonaws.com/omegatestgame/PathesSave.json",
                        _loadingFade);
                    break;
            }
        }
